      ...
      <stop offset="1" stop-color="#243BFF" stop-opacity="0"/>
                    ...
                    <div className="col-span-12 sm:col-span-7">
                        <ul className="flex flex-col gap-[20px] sm:gap-[22px] md:gap-[24px] lg:gap-[26px] xl:gap-[28px] 2xl:gap-[30px]">
                            {faqs.map((faq, index) => (
                                <li onClick={() => handleToggle(index)} key={index} className=" relative py-[18px] md:py-[21px] lg:py-[25px] xl:py-[28px] 2xl:py-[32px] px-[14px] sm:px-[16px] md:px-[18px] lg:px-[20px] xl:px-[22px] 2xl:px-[24px] border border-[#282D35] rounded-lg bg-[rgba(62,70,82,0.10)] hover:bg-[linear-gradient(0deg,rgba(122,127,153,0.20)_0%,rgba(37,38,46,0.14)_51.69%)] backdrop-blur-[47px] sm:border sm:rounded sm:backdrop-blur text-[#fff] manrope text-[14px] md:text-[16px] lg:text-[18px] xl:text-[20px] 2xl:text-[22px] w-full transition-all ease-in-out duration-1000">
                                    <div className="flex justify-between items-center font-semibold overflow-hidden">
                                        <p>{faq}</p>
                                        {
                                        toggle[index] ?
                                        <svg onClick={() => handleToggle(index)} xmlns="http://www.w3.org/2000/svg" width="40"  height="40" className="active:active:shadow-[0px_0px_53px_27px_#ffffff0a] active:bg-[#ffffff0a] rounded-full duration-300" viewBox="0 0 40 40" fill="none">
                                            <path className="duration-300" d="M11.668 28.3327L28.3346 11.666" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                            <path className="duration-300" d="M11.668 11.666H28.3346V28.3327" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                        :
                                        <svg onClick={() => handleToggle(index)} xmlns="http://www.w3.org/2000/svg" width="40" height="40" className="active:active:shadow-[0px_0px_53px_27px_#ffffff0a] active:bg-[#ffffff0a] rounded-full duration-300" viewBox="0 0 40 40" fill="none">
                                    ...
                                    className={`overflow-hidden duration-300  ${
                                        toggle[index] ? 'max-h-0 opacity-0' : 'max-h-10 opacity-100'
                                    }`}
                                    >
                                    <p className="text-[#ffffff99] text-ellipsis text-[12px] md:text-[14px] lg:text-[16px] xl:text-[18px] 2xl:text-[20px] font-normal leading-[160%] mt-[10px] sm:mt-[12px] md:mt-[14px] lg:mt-[16px] xl:mt-[18px] 2xl:mt-[15px] duration-1000">
                                        Each digital twin takes approximately 2 hours to film.</p>
                                    </div>
                                </li>
                            ))}
                        </ul>
